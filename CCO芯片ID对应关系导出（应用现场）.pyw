...
class MyMainWindow(QMainWindow, Ui_CCO_database_query):
    def __init__(self):
        ...
        self.sql_driver = r"DRIVER=Microsoft Access Driver (*.mdb, *.accdb);DBQ=C:/HiStudio-工装/3105集中器通信模块出厂检测/data/ndm/NoStaEquip/NoStaCCOCheckDB.mdb"
        # 创建数据库连接对象
        self.conn = pyodbc.connect(self.sql_driver)
        # 创建游标对象
        self.cur = self.conn.cursor()
        self.start_date = self.start_datetime.dateTime().toString("yyyy-MM-dd HH:mm")
        self.sqlstring = r"SELECT 总体测试结果,芯片ID值,模块ID值,日期 FROM NoStaTableCCOCheck where 芯片ID值<>'' and 日期>=? order by 日期 asc;"
    ...
    def click_query(self):
        ...
        try:
            self.lineEdit.setText(result_temp[0][1][-5:])
